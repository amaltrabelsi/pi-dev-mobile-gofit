...
public class ShowAll extends Form {
    ...
    private Component makeActualiteModel(Actualite actualite) {
        ...
        deleteBtn.addActionListener(action -> {
            ...
            dlg.add(BorderLayout.CENTER, new Label("Voulez vous vraiment supprimer ce actualite ?"));
            Button btnClose = new Button("Annuler");
            btnClose.addActionListener((ee) -> dlg.dispose());
            Button btnConfirm = new Button("Confirmer");
            btnConfirm.addActionListener(actionConf -> {
                int responseCode = ActualiteService.getInstance().delete(actualite.getId());

                if (responseCode == 200) {
                    currentActualite = null;
                    dlg.dispose();
                    actualiteModel.remove();
                    this.refreshTheme();
                } else {
                    Dialog.show("Erreur", "Erreur de suppression du actualite. Code d'erreur : " + responseCode, new Command("Ok"));
                }
            });
